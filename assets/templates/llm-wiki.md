...
This Pro version incorporates the foundation from [Andrej Karpathy's original LLM Wiki idea file](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) and extends it with what we learned running the pattern in production: what breaks at scale, what's missing, and what separates a wiki that stays useful from one that rots. It is intended to stand alone: the original model is summarized where needed, and the v2 extensions remain the main body of the document.

## What the original gets right

The core insight is correct: **stop re-deriving, start compiling.** RAG retrieves and forgets. A wiki accumulates and compounds. Instead of reassembling fragments from scratch on every query, the LLM incrementally builds a persistent, interlinked body of knowledge that sits between the raw sources and future answers.

The three-layer architecture works. **Raw sources** are the immutable evidence layer: articles, papers, transcripts, notes, images, or other source materials the LLM reads but does not rewrite. **The wiki** is the maintained knowledge layer: summaries, entity pages, concept pages, comparisons, syntheses, and cross-references that the LLM keeps current. **The schema** is the operating layer: the rules file that tells the LLM how the wiki is structured, what conventions to follow, and how to decide whether to ingest, update, link, lint, or ask for human judgment.

The operations (ingest, query, lint) cover the basics. **Ingest** turns sources into integrated wiki knowledge. **Query** answers from the wiki and can file valuable discoveries back into it. **Lint** checks the health of the knowledge base: contradictions, stale claims, orphan pages, missing links, duplicated concepts, and gaps that need new sources.
...
Keep the baseline roles clear. `index.md` is content-oriented: a human-readable catalog of important pages, grouped by category, with short descriptions and links. `log.md` is chronological: an append-only record of ingests, queries, lint passes, migrations, and other important changes. The index helps people and agents navigate; the log explains how the wiki evolved.
...
**Minimal viable wiki**: raw sources + wiki pages + index.md + log.md + a schema that describes ingest/query/lint workflows. This is roughly what the original describes. It works. Start here.
...
*This document extends [Andrej Karpathy's LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) with patterns proven in [agentmemory](https://github.com/rohitg00/agentmemory), a persistent memory engine for AI agents built on [iii-engine](https://github.com/iii-hq/iii). The exact directory structure, schema conventions, page formats, tools, and automation level should still depend on the user's domain and preferences. Start minimal, then add lifecycle, structure, automation, scale, and collaboration only when the wiki needs them.*
